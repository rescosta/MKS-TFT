...
const uint32_t touchBeepLength = 10;     			// beep length in ms
...
struct EEPROMData
{
	...
	EEPROMData() : magic(muggleVal) { }
	bool operator==(const EEPROMData& other);
	bool operator!=(const EEPROMData& other) { return !operator==(other); }
	bool IsValid() const;
...
bool EEPROMData::operator==(const EEPROMData& other)
{
...
void CalibrateTouch()
{
	...
	touch.init(DISPLAY_X, DISPLAY_Y, DefaultTouchOrientAdjust);				// initialize the driver and clear any existing calibration
	...
	DoTouchCalib(DISPLAY_X/2, touchCalibMargin, DISPLAY_X/2, DISPLAY_Y - 1 - touchCalibMargin, true, nvData.ymin);
	if (nvData.ymin >= 4096/2)
	{
		touch.adjustOrientation(ReverseY);
		nvData.ymin = 4095 - nvData.ymin;
	}
	DoTouchCalib(DISPLAY_X - touchCalibMargin - 1, DISPLAY_Y/2, touchCalibMargin, DISPLAY_Y/2, false, nvData.xmax);
	if (nvData.xmax < 4096/2)
	{
		touch.adjustOrientation(ReverseX);
		nvData.xmax = 4095 - nvData.xmax;
	}
	DoTouchCalib(DISPLAY_X/2, DISPLAY_Y - 1 - touchCalibMargin, DISPLAY_X/2, DISPLAY_Y - 1 - touchCalibMargin, true, nvData.ymax);
	DoTouchCalib(touchCalibMargin, DISPLAY_Y/2, touchCalibMargin, DISPLAY_Y/2, false, nvData.xmin);
...
{
	ShowLine;
	lastResponseTime = osKernelSysTick();
...
extern "C" int PanelDueMain(void)
{
	...
	if (nvData.IsValid())
	{
		...
		touch.init(DISPLAY_X, DISPLAY_Y, nvData.touchOrientation);
		touch.calibrate(nvData.xmin, nvData.xmax, nvData.ymin, nvData.ymax, touchCalibMargin);
		savedNvData = nvData;
	}
	else
	...
	lastPollTime = osKernelSysTick() - printerPollInterval;	// allow a poll immediately
	...
	UI::ShowDefaultPage();
	lastResponseTime = osKernelSysTick();	// pretend we just received a response
	...
	for (;;)
	{
		...
		if (osKernelSysTick() - lastTouchTime >= ignoreTouchTime)
		{
